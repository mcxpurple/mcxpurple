        ...
        if mod_path.exists():
            try:
                with open(mod_path, "r", encoding="utf-8") as mf:
                    ...
                    # 這是溫和的淨化步驟：只移除零寬度字符和 BOM
                    # 避免移除可能合法的控制字符或非標準但有效的 Unicode 符號
                    cleaned_content = re.sub(r'[\u200b\u200c\u200d\uFEFF]', '', raw_content) # 移除了 \uFFFD 和 \u0000-\u001F，只針對零寬度字符和BOM
                    
                    # 處理以 '?' 開頭的行，避免被誤解析為隱式鍵
                    # 這個步驟保留，因為它處理的是 YAML 語法本身的歧義，而不是字符污染
                    preprocessed_content = re.sub(r'^\?([^\s]?)', r'# ?\1', cleaned_content, flags=re.MULTILINE)

                    temp_parsed_data = yaml.safe_load(preprocessed_content)
                    ...
                    if isinstance(temp_parsed_data, dict) and 'sections_raw' in temp_parsed_data:
                        ...
                        # 對 sections_raw 的內容進行第二次淨化和預處理 (同樣使用溫和版)
                        cleaned_sections_content = re.sub(r'[\u200b\u200c\u200d\uFEFF]', '', raw_sections_content)
                        preprocessed_sections_content = re.sub(r'^\?([^\s]?)', r'# ?\1', cleaned_sections_content, flags=re.MULTILINE)
                        
                        parsed_from_sections_raw = yaml.safe_load(preprocessed_sections_content)
